...
class MemoryCache : public LruCache<shash::Any, kvstore::MemoryBuffer> {
 public:
  explicit MemoryCache(unsigned int cache_size, perf::Statistics *statistics) :
...
class KvStore : SingleCopy {
 public:
  ...
  virtual int64_t Pread(const shash::Any &id, void *buf, uint64_t size, uint64_t offset) = 0;
  virtual bool Commit(const shash::Any &id, void *buf, uint64_t size) = 0;
...
class MemoryKvStore : KvStore {
 public:
  ...
  virtual int64_t Pread(const shash::Any &id, void *buf, uint64_t size, uint64_t offset);
  virtual bool Commit(const shash::Any &id, void *buf, uint64_t size);
